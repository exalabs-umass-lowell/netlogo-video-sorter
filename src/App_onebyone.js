...
export default function VideoPairApp() {
  ...
  var start = performance.now(); // is the survey starting?
...
function shuffleNoConsecutive(arr) { // important to ensure that the same behavior of swarm is not shown more than once, ChatGPT provided function
  ...
  if (preSurvey) {
    return (
      <div style={{ justifyContent: 'center', alignItems: 'center', flexDirection: 'column', display: 'flex', width: '100%', minHeight: '100vh', overflowX: 'hidden', background: `linear-gradient(rgba(0,0,0,0.45), rgba(0,0,0,0.45)), radial-gradient(circle at center, rgba(0,0,204,0.6) 50%, rgba(179,217,255, 0.6) 100%), url('swarm-background.jpg')`, transition: 'background-image 0.5s ease', backgroundSize: '100% 100%, 100% 100%, contain', backgroundPosition: 'center, center, center', backgroundRepeat: 'no-repeat, no-repeat, no-repeat', backgroundBlendMode: 'multiply',}}>
        <InstructionBoard msgs={presurvey_messages} />
        <Button sx= {{ gap: '150px', margin: '100px 0'}} variant="contained" onClick={() => {
           setPreSurvey(false);
           setInitSurvey(true);
    ...
    return (
      ...
      <div style={{ justifyContent: 'center', alignItems: 'center', flexDirection: 'column', display: 'flex', width: '100%', minHeight: '100vh', overflowX: 'hidden', background: `linear-gradient(rgba(0,0,0,0.45), rgba(0,0,0,0.45)), radial-gradient(circle at center, rgba(0,0,204,0.6) 50%, rgba(179,217,255, 0.6) 100%), url('swarm-background.jpg')`, transition: 'background-image 0.5s ease', backgroundSize: '100% 100%, 100% 100%, contain', backgroundPosition: 'center, center, center', backgroundRepeat: 'no-repeat, no-repeat, no-repeat', backgroundBlendMode: 'multiply',}}>
        ...
        <Box sx={{ justifyContent: 'center', alignItems: 'center', position: 'relative',}}>
           <Typography sx={{fontWeight: 'bold', fontSize: '50px', alignItems: 'center', justifyContent: 'center', display: 'flex', position: 'relative', margin: '50px', fontFamily: "'Orbitron', monospace", }}>Survey complete</Typography>
        </Box>
        <Box sx={{ padding: '5px', justifyContent: 'center', alignItems: 'center', position: 'relative',}}>
          <Button onClick={ () => setDisplay(1) } sx={{ '&:hover': { backgroundColor: 'rgba(150, 220, 255, 0.9)', fontSize: '25px', marginLeft: '30px', marginRight: '30px', fontWeight:'bold' }, fontSize: '25px', marginLeft: '30px', marginRight: '30px', color: '#FFF', fontWeight:'bold', }}>Show complexity rankings</Button>
          <Button onClick={ () => setDisplay(0) } sx={{ '&:hover': { backgroundColor: 'rgba(150, 220, 255, 0.9)', fontSize: '25px', marginLeft: '30px', marginRight: '30px', fontWeight:'bold', }, fontSize: '25px', marginLeft: '30px', marginRight: '30px', color: '#FFF', fontWeight:'bold', }}>Show choice results</Button>
        </Box>
        <Button sx= {{ gap: '50px', margin: '50px 0', '&:hover': { backgroundColor: 'rgba(150, 220, 255, 0.9)' }, fontSize: '25px', }} onClick={restart}>Restart</Button>
      {display ? (
       <>
        <pre style={{ whiteSpace: "pre-wrap", maxHeight: 'flex', overflow: "auto", justifyContent: 'center', alignItems: 'center', }}>
        {Object.entries(rankedVideos).map(
           ([rank, index]) => (<p><RankCard item={rank} index={index+1}/></p>)
        )}
        </pre>
        <DoubleArrow xpos='3%' ypos='59%' />
       </>
      ) : (
       <>
        <pre style={{ whiteSpace: "pre-wrap", maxHeight: 'flex', overflow: "auto" }}>
        {
          chosenVids.map((chosen, i) => (
           <p><ChoiceCard chosen={chosenVids[i]} notChosen={notChosenVids[i]} time={times[i]}/></p>
          ))
        }
        </pre>
       </>
      )}
      <EmailBox xpos='95%' ypos='45%'/>
      </div>
      </>
  ...
  return (
    ...
    <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial" }}>
      <div>
      <div style={{ fontSize: '50px', fontWeight: 'bold', color: 'white', backgroundColor: '#001EB3', textAlign: 'center', padding: '50px', width: 'flex', }}>Video Pairwise Sorter</div>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 25, padding: '30px', color: '#FFF' }}>
        <span>Select the video you think is{' '}</span>&nbsp;
      ...
      <Box sx={{ padding: '20px 20px', borderRadius: '10px', gap: '50px 50px', }}>
        <button style={{ backgroundColor: '#B3E5FF', padding: '20px 20px', borderRadius: '10px', }} onClick={restart}>Restart</button>
      </Box>
      <Box sx={{ border: '5px solid #0080FF', fontSize: 30, borderRadius: '10px', padding: '30px', gap: '50px',  alignItems: 'center', justifyContent: 'center', display: 'inline-block', backgroundColor: 'rgba(179, 217, 255, 0.67)', }}>
         <div style={{ justifyContent: 'center', alignItems: 'center', display: 'flex', }}>Progress:</div> 
         <div style={{ justifyContent: 'center', alignItems: 'center', display: 'flex', }}><strong>{chosenVids.length} / {surveyVids.length - 1}</strong></div> 
         <div style={{ justifyContent: 'center', alignItems: 'center', display: 'flex', }}>completed</div>
      </Box>
      </Box>
      ...
      <HelpBox headline="How to determine which video is more complex?" msg="The more complex video is one in which you don't see a clear pattern" xpos='58%' ypos='90%' />
...
function Timerbox({ start, time_to_choose }) {
  ...
  useEffect(() => {
    ...
    console.log("timer: "+String(timer)+", start: "+String(start));
...
function VideoCard({ item, onChoose, position = "left" }) {
  ...
  return (
    ...
    >
      ...
      />
      <div style={{ marginTop: 8, fontSize: 12, color: "#000" }}><strong>URL: </strong>{item.id}</div>
    </div>
  );
}
...
function DoubleArrow({ xpos, ypos }) {
  ...
  <Box sx={{
     display: 'inline-block',
     flexDirection: 'column',
     top: `calc(${ypos} + ${Object.keys(rankedVideos).length/2} - 40%)`,
